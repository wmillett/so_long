...
/*   Updated: 2023/05/23 20:44:42 by wmillett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exit_open(t_game *gpack)
{
	...
	{
		count += ft_strchr_count(gpack->mpack, gpack->mpack.map2[i], 'C');
		i++;
	}
	if (count == 0)
	{
...
static void	put_image(t_game *gpack, mlx_image_t *mc)
{
	...
	check_render(gpack->temps.check, gpack, 0);
}
...
mlx_image_t	*render_mc(t_game *gpack, int move, int y, int x)
{
	...
	if (move != 'F'
		...
		gpack->mpack.map2 = render_items(gpack, 0, 0, 0);
	else if (move == 'F')
		gpack->mpack.map2 = render_items(gpack, 'F', 0, 0);
	exit_open(gpack);
	print_move(move);
	return (mc);
}
...
t_game	*render_static_map(t_game *gpack, int i)
{
	...
	render_mc(gpack, 'F', gpack->cpack.pos.y, gpack->cpack.pos.x);
	return (gpack);
}
