...
/*   Updated: 2023/04/18 21:37:34 by wmillett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	objects(t_map mpack)
{
	...
	while (objects[i])
	{
		...
		j = -1;
		while (++j < mpack.size.y)
			count += ft_strchr_count(mpack, mpack.map[j], objects[i]);
		if (count != 1 && objects[i] == 'P')
			printerror1(1, mpack, count);
		if (count != 1 && objects[i] == 'E')
			printerror1(2, mpack, count);
		if (objects[i] == 'C' && count <= 0)
			printerror1(3, mpack, count);
